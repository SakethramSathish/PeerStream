...
action is a coroutine handed to the bridge and forgotten ΓÇö no widget in this
...
1. **Measurements** arrive by pull: the bridge's pump emits a snapshot every
   ...
   are not visible are not redrawn ΓÇö a hidden widget has nothing to show for
   the work.
...
class MainWindow(QMainWindow):
    ...
    Args:
        bridge: The Qt Γåö asyncio bridge.
    ...
    #: runs on the engine thread, which has no Qt event loop ΓÇö so the result is
    ...
    def _on_torrent_chosen(self, torrent: Torrent) -> None:
        directory = self._config.storage.download_directory
        ...
        self._run(add, on_done=lambda future: self._on_added(future, torrent.name))

    def _on_magnet_chosen(self, magnet: MagnetUri) -> None:
        ...
        self._run(add, on_done=lambda future: self._on_added(future, name))

    def _on_added(self, future: Future[Any], name: str) -> None:
    ...
    def _run(
        ...
        *,
        ...
        engine's thread ΓÇö so the outcome crosses back as a queued signal
    ...
    def _refresh_dht(self) -> None:
        ...
        The routing table changes on its own ΓÇö nodes answer, go quiet, get
        replaced ΓÇö so the panel is polled rather than waiting for an event that
